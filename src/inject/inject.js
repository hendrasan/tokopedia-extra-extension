...
function init() {
  ...
  var items = document.querySelectorAll('._33JN2R1i, ._2zW9OOW2, .product-card-category, .product-card');
    ...
    var observer = new MutationObserver(function(mutations) {
      mutations.forEach(function(mutation) {
        Array.prototype.slice.call(mutation.addedNodes).forEach(function(el) {
          if (el.classList.contains('_33JN2R1i') || el.classList.contains('_2zW9OOW2') || el.classList.contains('product-card-category') || el.classList.contains('product-card')) {
            getItemStat(el);
          }
        });
      });
...
function getItemStat(item) {
  ...
  axios.get(url)
    .then(function (response) {
      ...
      var el = item.querySelector('.vlEGRFVq, ._2AXi12hA, .product-summary, .detail');
